...
const filters = reactive({ pageNum: 1, pageSize: 10, name: '' })
...
const loadData = async () => {
  state.loading = true
  try {
    const data = await getTenantRoleList({ pageNum: filters.pageNum, pageSize: filters.pageSize, name: filters.name || undefined })
    ...
    rows.value = []
    state.total = 0
    ElMessage.error(error?.message || '小程序角色列表加载失败')
  } finally {
    state.loading = false
  }
}
...
<style scoped>
.module-page { display: flex; flex-direction: column; gap: 12px; padding: 0 20px; }
.search-form { display: grid; grid-template-columns: repeat(3, minmax(180px, 1fr)); gap: 12px 20px; }
.search-form :deep(.el-form-item) { margin-bottom: 0; }
.search-form :deep(.el-input) { width: 100%; }
.search-actions { grid-column: 1 / -1; justify-self: end; }
