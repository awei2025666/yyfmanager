...
const queryPayload = () => {
  ...
  return {
    ...
    outTenantName: filters.transferUnit || undefined,
...
const loadStatistics = async () => {
  try {
    const data = await getTenantOutsourceCraftStatistics()
...
const loadData = async () => {
  state.loading = true
  try {
    const data = await getTenantOutsourceCraftList(queryPayload())
    ...
    rows.value = []
    state.total = 0
    ElMessage.error(error?.message || '外协工艺列表加载失败')
  } finally {
    state.loading = false
  }
}
...
const normalizeDetail = (payload = {}, fallback = {}) => {
  ...
  return {
    ...
    transferUnit: fallback.transferUnit,
...
const openDetail = async (row) => {
  ...
  try {
    ...
    ElMessage.error(error?.message || '外协工艺详情加载失败')
  } finally {
...
<template>
  <div class="module-page">
    ...
    <el-dialog v-model="detailVisible" title="外协工艺详情" width="1080px">
      <div v-loading="state.detailLoading" class="detail-wrap">
        <dl v-if="currentRow" class="detail-grid">
          ...
          <div><dt>转单单位</dt><dd>{{ currentRow.transferUnit || '-' }}</dd></div>
