	...
	<view class="transit-page">
		...
		<view class="list">
			...
			<view class="order-card" v-for="item in list" :key="item.id" @click="toDetail(item)">
				...
				<button v-if="!isDone(item)" class="complete-btn" @click.stop="complete(item)">已完成配送</button>
			</view>
		</view>
	</view>
</template>
...
const complete = async item => {
	try {
		await uni.$api.completeDelivery({ id: item.id })
...
const toDetail = item => {
	if (!item.id) return
	uni.navigateTo({ url: `/pages/deliveryDetail/index?id=${item.id}` })
}
